...
sync-db:
	...
	ssh portfolio "export PATH=\$$PATH:/snap/bin && cd /home/taher/portfolio_v2 && docker compose -f docker-compose.prod.yml exec -T db pg_dump -U ${PROD_USERNAME} -d ${PROD_DATABASE_NAME}> /tmp/backup_$$DATE.sql" && \
	echo "=== Copying to local ===" && \
	scp portfolio:/tmp/backup_$$DATE.sql ./backup_$$DATE.sql && \
	echo "=== Restoring local DB ===" && \
	docker compose exec -T db psql -U ${POSTGRES_USER} -d ${POSTGRES_DB} < backup_$$DATE.sql && \
