<script>
	...
	let coffeeData = $derived.by(() => {
		...
		const sorted = filtered.sort((a, b) => {
			const sortKey =
				selectedLabel === "Date Drank"
					? "Date Drank Raw"
					: selectedLabel;
			const labelA = a[sortKey];
			const labelB = b[sortKey];

			if (labelA instanceof Date)
				return labelA.getTime() - labelB.getTime();
...
<div class="h-64 w-full max-w-5xl items-center justify-center mx-auto mb-6">
	<canvas class="bg-secondary text-md text-font" use:chartRender={coffeeData}
	></canvas>
</div>
<CoffeeTable
